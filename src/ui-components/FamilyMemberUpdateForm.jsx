...
export default function FamilyMemberUpdateForm(props) {
  const {
    id: idProp,
    familyMember: familyMemberModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = familyMemberRecord
      ? { ...initialValues, ...familyMemberRecord }
      : initialValues;
    ...
    setMobileNumber(
      typeof cleanValues.mobileNumber === "string" ||
        cleanValues.mobileNumber === null
        ? cleanValues.mobileNumber
        : JSON.stringify(cleanValues.mobileNumber)
    );
    setCurrentOccupation(cleanValues.currentOccupation);
    setPlaceOfBirth(cleanValues.placeOfBirth);
    setAddress(cleanValues.address);
    setCityTown(cleanValues.cityTown);
    setPincode(cleanValues.pincode);
    setFatherName(cleanValues.fatherName);
    setChildren(cleanValues.children ?? []);
    setCurrentChildrenValue("");
    setMaritalStatus(cleanValues.maritalStatus);
    setSibling(cleanValues.sibling ?? []);
    setCurrentSiblingValue("");
    setErrors({});
  };
  const [familyMemberRecord, setFamilyMemberRecord] = React.useState(
    familyMemberModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(FamilyMember, idProp)
        : familyMemberModelProp;
      setFamilyMemberRecord(record);
    };
    queryData();
  }, [idProp, familyMemberModelProp]);
  React.useEffect(resetStateValues, [familyMemberRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            FamilyMember.copyOf(familyMemberRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "FamilyMemberUpdateForm")}
      {...rest}
    >
      <TextField
        label="First name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || familyMemberModelProp)}
